...
- **The thermal wall — waste heat management:** The problem splits by phase. During the acceleration burn, regenerative cooling uses the propellant itself as the heat sink — waste heat is absorbed by propellant passing through engine structure and exits with the exhaust. No separate radiator required for this phase. During deceleration, the burn is an order of magnitude slower and gentler — lower thrust over 20-40 years produces substantially less heat per unit time. Static radiators sized for the deceleration phase load are much lighter than radiators sized for peak acceleration. The thermal wall is less severe than it first appears when the phases are treated separately. What remains: the specific radiator requirement for the deceleration phase, and whether regenerative cooling is fully sufficient for the acceleration phase at the drive architectures under consideration.
...
1. **Solan change the mission profile completely:** No biological tolerance constraint for Veros or Carbon-Os. Acceleration loads lethal to biosapients are operational parameters for both. The mission profile designed around a Solan in hardened substrate looks nothing like a crewed mission.
