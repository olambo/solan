...
The quoted 150 tonne figure refers to estimated cruise mass, not initial launch mass. After acceleration burn completion and propellant expenditure, the Voidfarer is expected to retain approximately 150 tonnes at transit velocity. Initial departure mass would necessarily be higher, likely exceeding 200 tonnes depending on propulsion configuration. Kinetic energy estimates use the cruise mass value. The working mass scale is closer to a large aircraft than a naval vessel or megastructure, though the true figure depends entirely on propulsion and shielding requirements not yet solved. A verified mass budget has not been derived; the propellant fraction depends on exhaust velocity assumptions that require a specific drive architecture to calculate. 150 tonnes cruise mass is a working floor, not an engineering result. Unless otherwise specified, quoted Voidfarer mass figures refer to cruise mass: the retained vehicle mass at interstellar transit velocity after completion of the primary acceleration phase.
...
- **VoidForge rotation:** A turntable or large bearing is built into the interface between the VoidForge and the Voidfarer structure — the connection that exists anyway, made rotatable. Electric motor drive rotates the VoidForge slowly after the acceleration burn. Solan engineers disconnect fuel lines before rotation, supervise, then reconnect once the VoidForge is in its deceleration orientation. The turntable then locks mechanically. That locked interface becomes the primary structural joint for the 500-year cruise — exactly the load case a purpose-built bearing is designed to handle. The rotation happens once, at the best possible moment, with crew oversight and no time pressure.
